...
exports.PathConstraint = exports.IKConstraint = exports.Constraint = void 0;
...
class Constraint extends BaseObject_1.BaseObject {
    _onClear() {
        this._armature = null; //
        this._target = null; //
        this._root = null; //
...
Constraint._helpMatrix = new Matrix_1.Matrix();
Constraint._helpTransform = new Transform_1.Transform();
Constraint._helpPoint = new Point_1.Point();
/**
 * @internal
 */
class IKConstraint extends Constraint {
    ...
    _computeA() {
        ...
        global.rotation += Transform_1.Transform.normalizeRadian(radian - global.rotation) * this._weight;
    ...
    _computeB() {
        ...
        const dR = Transform_1.Transform.normalizeRadian(radianA - rawRadianA);
        ...
        global.rotation = parentGlobal.rotation + rawRadian - rawParentRadian + Transform_1.Transform.normalizeRadian(radianB - dR - rawRadian) * this._weight;
        global.toMatrix(globalTransformMatrix);
    }
...
class PathConstraint extends Constraint {
    ...
    update() {
        ...
        if (rotateOffset === 0) {
            ...
            if (bone !== null) {
                const matrix = bone.globalTransformMatrix;
                rotateOffset *= matrix.a * matrix.d - matrix.b * matrix.c > 0 ? Transform_1.Transform.DEG_RAD : -Transform_1.Transform.DEG_RAD;
            }
        }
        //
        ...
        for (let i = 0, p = 3; i < boneCount; i++, p += 3) {
            ...
            if (rotateMix > 0) {
                ...
                if (r > Transform_1.Transform.PI) {
                    r -= Transform_1.Transform.PI_D;
                }
                else if (r < -Transform_1.Transform.PI) {
                    r += Transform_1.Transform.PI_D;
                }
                r *= rotateMix;
                cos = Math.cos(r);
