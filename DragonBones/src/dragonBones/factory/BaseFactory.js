...
exports.BuildArmaturePackage = exports.BaseFactory = void 0;
const Armature_1 = require("../armature/Armature");
const Bone_1 = require("../armature/Bone");
const Constraint_1 = require("../armature/Constraint");
const Surface_1 = require("../armature/Surface");
const BaseObject_1 = require("../core/BaseObject");
const EventObject_1 = require("../event/EventObject");
const BinaryDataParser_1 = require("../parser/BinaryDataParser");
const ObjectDataParser_1 = require("../parser/ObjectDataParser");
class BaseFactory {
    /**
    ...
    constructor(dataParser = null) {
        ...
        if (BaseFactory._objectParser === null) {
            BaseFactory._objectParser = new ObjectDataParser_1.ObjectDataParser();
        }
        if (BaseFactory._binaryParser === null) {
            BaseFactory._binaryParser = new BinaryDataParser_1.BinaryDataParser();
        }
    ...
    _buildBones(dataPackage, armature) {
        for (const boneData of dataPackage.armature.sortedBones) {
            const bone = BaseObject_1.BaseObject.borrowObject(boneData.type === 0 /* Bone */ ? Bone_1.Bone : Surface_1.Surface);
    ...
    _buildConstraints(dataPackage, armature) {
        ...
        for (let k in constraints) {
            ...
            switch (constraintData.type) {
                case 0 /* IK */:
                    const ikConstraint = BaseObject_1.BaseObject.borrowObject(Constraint_1.IKConstraint);
                    ikConstraint.init(constraintData, armature);
                    armature._addConstraint(ikConstraint);
                    break;
                case 1 /* Path */:
                    const pathConstraint = BaseObject_1.BaseObject.borrowObject(Constraint_1.PathConstraint);
                    pathConstraint.init(constraintData, armature);
                    armature._addConstraint(pathConstraint);
                    break;
                default:
                    const constraint = BaseObject_1.BaseObject.borrowObject(Constraint_1.IKConstraint);
                    ...
                    if (!childArmature.inheritAnimation) {
                        ...
                        if (actions.length > 0) {
                            for (const action of actions) {
                                ...
                                EventObject_1.EventObject.actionDataToInstance(action, eventObject, slot.armature);
    ...
    replaceAnimation(armature, armatureData, isOverride = true) {
        ...
        for (const slot of armature.getSlots()) {
            ...
            for (const display of slot.displayList) {
                if (display instanceof Armature_1.Armature) {
