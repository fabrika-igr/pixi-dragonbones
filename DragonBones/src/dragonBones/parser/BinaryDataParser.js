...
exports.BinaryDataParser = void 0;
...
const ObjectDataParser_1 = require("./ObjectDataParser");
class BinaryDataParser extends ObjectDataParser_1.ObjectDataParser {
    _inRange(a, min, max) {
        return min <= a && a <= max;
    }
    _decodeUTF8(data) {
    ...
    _parseBinaryTimeline(type, offset, timelineData = null) {
        const timeline = timelineData !== null ? timelineData : BaseObject_1.BaseObject.borrowObject(AnimationData_1.TimelineData);
        timeline.type = type;
    ...
    _parseAnimation(rawData) {
        ...
        animation.blendType = DataParser_1.DataParser._getAnimationBlendType(ObjectDataParser_1.ObjectDataParser._getString(rawData, DataParser_1.DataParser.BLEND_TYPE, ""));
        animation.frameCount = ObjectDataParser_1.ObjectDataParser._getNumber(rawData, DataParser_1.DataParser.DURATION, 0);
        animation.playTimes = ObjectDataParser_1.ObjectDataParser._getNumber(rawData, DataParser_1.DataParser.PLAY_TIMES, 1);
        animation.duration = animation.frameCount / this._armature.frameRate; // float
        animation.fadeInTime = ObjectDataParser_1.ObjectDataParser._getNumber(rawData, DataParser_1.DataParser.FADE_IN_TIME, 0.0);
        animation.scale = ObjectDataParser_1.ObjectDataParser._getNumber(rawData, DataParser_1.DataParser.SCALE, 1.0);
        animation.name = ObjectDataParser_1.ObjectDataParser._getString(rawData, DataParser_1.DataParser.NAME, DataParser_1.DataParser.DEFAULT_NAME);
        if (animation.name.length === 0) {
            animation.name = DataParser_1.DataParser.DEFAULT_NAME;
        }
        // Offsets.
        const offsets = rawData[DataParser_1.DataParser.OFFSET];
        ...
        if (DataParser_1.DataParser.ACTION in rawData) {
            animation.actionTimeline = this._parseBinaryTimeline(0 /* Action */, rawData[DataParser_1.DataParser.ACTION]);
        }
        if (DataParser_1.DataParser.Z_ORDER in rawData) {
            animation.zOrderTimeline = this._parseBinaryTimeline(1 /* ZOrder */, rawData[DataParser_1.DataParser.Z_ORDER]);
        }
        if (DataParser_1.DataParser.BONE in rawData) {
            const rawTimeliness = rawData[DataParser_1.DataParser.BONE];
        ...
        if (DataParser_1.DataParser.SLOT in rawData) {
            const rawTimeliness = rawData[DataParser_1.DataParser.SLOT];
            for (let k in rawTimeliness) {
                const rawTimelines = rawTimeliness[k];
        ...
        if (DataParser_1.DataParser.CONSTRAINT in rawData) {
            const rawTimeliness = rawData[DataParser_1.DataParser.CONSTRAINT];
            for (let k in rawTimeliness) {
                const rawTimelines = rawTimeliness[k];
        ...
        if (DataParser_1.DataParser.TIMELINE in rawData) {
            ...
            for (const rawTimeline of rawTimelines) {
                const timelineOffset = ObjectDataParser_1.ObjectDataParser._getNumber(rawTimeline, DataParser_1.DataParser.OFFSET, 0);
                if (timelineOffset >= 0) {
                    const timelineType = ObjectDataParser_1.ObjectDataParser._getNumber(rawTimeline, DataParser_1.DataParser.TYPE, 0 /* Action */);
                    const timelineName = ObjectDataParser_1.ObjectDataParser._getString(rawTimeline, DataParser_1.DataParser.NAME, "");
                    let timeline = null;
                    if (timelineType === 40 /* AnimationProgress */ && animation.blendType !== 0 /* None */) {
                        ...
                        animaitonTimeline.x = ObjectDataParser_1.ObjectDataParser._getNumber(rawTimeline, DataParser_1.DataParser.X, 0.0);
                        animaitonTimeline.y = ObjectDataParser_1.ObjectDataParser._getNumber(rawTimeline, DataParser_1.DataParser.Y, 0.0);
                    }
    ...
    _parseGeometry(rawData, geometry) {
        geometry.offset = rawData[DataParser_1.DataParser.OFFSET];
