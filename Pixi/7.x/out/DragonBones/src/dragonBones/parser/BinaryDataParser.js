...
export class BinaryDataParser extends ObjectDataParser {
    ...
    _parseAnimation(rawData) {
        ...
        if (DataParser.ACTION in rawData) {
            animation.actionTimeline = this._parseBinaryTimeline(0 /* Action */, rawData[DataParser.ACTION]);
        }
        if (DataParser.Z_ORDER in rawData) {
            animation.zOrderTimeline = this._parseBinaryTimeline(1 /* ZOrder */, rawData[DataParser.Z_ORDER]);
        }
