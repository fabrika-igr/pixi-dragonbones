...
import { EllipseBoundingBoxData, PolygonBoundingBoxData, RectangleBoundingBoxData } from "../model/BoundingBoxData";
import { CanvasData } from "../model/CanvasData";
import { IKConstraintData, PathConstraintData } from "../model/ConstraintData";
import { ArmatureDisplayData, BoundingBoxDisplayData, ImageDisplayData, MeshDisplayData, PathDisplayData, WeightData } from "../model/DisplayData";
...
export class ObjectDataParser extends DataParser {
    ...
    _parseActionDataInFrame(rawData, frameStart, bone, slot) {
        if (DataParser.EVENT in rawData) {
            this._mergeActionFrame(rawData[DataParser.EVENT], frameStart, 10 /* Frame */, bone, slot);
        }
        if (DataParser.SOUND in rawData) {
            this._mergeActionFrame(rawData[DataParser.SOUND], frameStart, 11 /* Sound */, bone, slot);
        }
        if (DataParser.ACTION in rawData) {
            this._mergeActionFrame(rawData[DataParser.ACTION], frameStart, 0 /* Play */, bone, slot);
        }
        if (DataParser.EVENTS in rawData) {
            this._mergeActionFrame(rawData[DataParser.EVENTS], frameStart, 10 /* Frame */, bone, slot);
        }
        if (DataParser.ACTIONS in rawData) {
            this._mergeActionFrame(rawData[DataParser.ACTIONS], frameStart, 0 /* Play */, bone, slot);
        }
    }
    ...
    _parseArmature(rawData, scale) {
        ...
        if (DataParser.TYPE in rawData && typeof rawData[DataParser.TYPE] === "string") {
            ...
            armature.type = ObjectDataParser._getNumber(rawData, DataParser.TYPE, 0 /* Armature */);
        }
        ...
        if (DataParser.DEFAULT_ACTIONS in rawData) {
            const actions = this._parseActionData(rawData[DataParser.DEFAULT_ACTIONS], 0 /* Play */, null, null);
            for (const action of actions) {
        ...
        if (DataParser.ACTIONS in rawData) {
            const actions = this._parseActionData(rawData[DataParser.ACTIONS], 0 /* Play */, null, null);
            for (const action of actions) {
    ...
    _parseBone(rawData) {
        ...
        if (DataParser.TYPE in rawData && typeof rawData[DataParser.TYPE] === "string") {
            ...
            type = ObjectDataParser._getNumber(rawData, DataParser.TYPE, 0 /* Bone */);
        }
        if (type === 0 /* Bone */) {
    ...
    _parseSlot(rawData, zOrder) {
        ...
        if (DataParser.BLEND_MODE in rawData && typeof rawData[DataParser.BLEND_MODE] === "string") {
            ...
            slot.blendMode = ObjectDataParser._getNumber(rawData, DataParser.BLEND_MODE, 0 /* Normal */);
        }
        ...
        if (DataParser.ACTIONS in rawData) {
            this._slotChildActions[slot.name] = this._parseActionData(rawData[DataParser.ACTIONS], 0 /* Play */, null, null);
        }
        return slot;
    }
    _parseSkin(rawData) {
    ...
    _parseAnimation(rawData) {
        ...
        if (DataParser.Z_ORDER in rawData) {
            this._animation.zOrderTimeline = this._parseTimeline(rawData[DataParser.Z_ORDER], null, DataParser.FRAME, 1 /* ZOrder */, 0 /* Step */, 0, this._parseZOrderFrame);
        }
        if (DataParser.BONE in rawData) {
        ...
        if (DataParser.FFD in rawData) {
            ...
            for (const rawTimeline of rawTimelines) {
                ...
                const timeline = this._parseTimeline(rawTimeline, null, DataParser.FRAME, 22 /* SlotDeform */, 2 /* Float */, 0, this._parseSlotDeformFrame);
                if (timeline !== null) {
        ...
        if (DataParser.IK in rawData) {
            ...
            for (const rawTimeline of rawTimelines) {
                ...
                const timeline = this._parseTimeline(rawTimeline, null, DataParser.FRAME, 30 /* IKConstraint */, 1 /* Int */, 2, this._parseIKConstraintFrame);
                if (timeline !== null) {
        ...
        if (DataParser.TIMELINE in rawData) {
            ...
            for (const rawTimeline of rawTimelines) {
                const timelineType = ObjectDataParser._getNumber(rawTimeline, DataParser.TYPE, 0 /* Action */);
                ...
                switch (timelineType) {
                    ...
                    case 50 /* Surface */: {
                        ...
                        timeline = this._parseTimeline(rawTimeline, null, DataParser.FRAME, timelineType, 2 /* Float */, 0, this._parseDeformFrame);
                        this._geometry = null; //
                        break;
                    }
                    case 22 /* SlotDeform */: {
                    ...
                    case 21 /* SlotColor */:
                        timeline = this._parseTimeline(rawTimeline, null, DataParser.FRAME, timelineType, 1 /* Int */, 1, this._parseSlotColorFrame);
                        break;
                }
                if (timeline !== null) {
                    switch (timelineType) {
    ...
    _parseTimeline(rawData, rawFrames, framesKey, timelineType, frameValueType, frameValueCount, frameParser, timeline = null) {
        ...
        if (rawData !== null) {
            this._timelineArray[timelineOffset + 0 /* TimelineScale */] = Math.round(ObjectDataParser._getNumber(rawData, DataParser.SCALE, 1.0) * 100);
            this._timelineArray[timelineOffset + 1 /* TimelineOffset */] = Math.round(ObjectDataParser._getNumber(rawData, DataParser.OFFSET, 0.0) * 100);
        }
        else {
    ...
    _parseBoneTimeline(rawData) {
        ...
        if (DataParser.TRANSLATE_FRAME in rawData) {
            this._frameDefaultValue = 0.0;
            this._frameValueScale = 1.0;
            const timeline = this._parseTimeline(rawData, null, DataParser.TRANSLATE_FRAME, 11 /* BoneTranslate */, 2 /* Float */, 2, this._parseDoubleValueFrame);
            if (timeline !== null) {
        ...
        if (DataParser.ROTATE_FRAME in rawData) {
            this._frameDefaultValue = 0.0;
            this._frameValueScale = 1.0;
            const timeline = this._parseTimeline(rawData, null, DataParser.ROTATE_FRAME, 12 /* BoneRotate */, 2 /* Float */, 2, this._parseBoneRotateFrame);
            if (timeline !== null) {
                this._animation.addBoneTimeline(bone.name, timeline);
            }
        }
        if (DataParser.SCALE_FRAME in rawData) {
            this._frameDefaultValue = 1.0;
            this._frameValueScale = 1.0;
            const timeline = this._parseTimeline(rawData, null, DataParser.SCALE_FRAME, 13 /* BoneScale */, 2 /* Float */, 2, this._parseBoneScaleFrame);
            if (timeline !== null) {
                this._animation.addBoneTimeline(bone.name, timeline);
            }
        }
        if (DataParser.FRAME in rawData) {
            const timeline = this._parseTimeline(rawData, null, DataParser.FRAME, 10 /* BoneAll */, 2 /* Float */, 6, this._parseBoneAllFrame);
            if (timeline !== null) {
                this._animation.addBoneTimeline(bone.name, timeline);
            }
        }
    ...
    _parseSlotTimeline(rawData) {
        ...
        if (DataParser.DISPLAY_FRAME in rawData) {
            displayTimeline = this._parseTimeline(rawData, null, DataParser.DISPLAY_FRAME, 20 /* SlotDisplay */, 0 /* Step */, 0, this._parseSlotDisplayFrame);
        }
        else {
            displayTimeline = this._parseTimeline(rawData, null, DataParser.FRAME, 20 /* SlotDisplay */, 0 /* Step */, 0, this._parseSlotDisplayFrame);
        }
        if (DataParser.COLOR_FRAME in rawData) {
            colorTimeline = this._parseTimeline(rawData, null, DataParser.COLOR_FRAME, 21 /* SlotColor */, 1 /* Int */, 1, this._parseSlotColorFrame);
        }
        else {
            colorTimeline = this._parseTimeline(rawData, null, DataParser.FRAME, 21 /* SlotColor */, 1 /* Int */, 1, this._parseSlotColorFrame);
        }
    ...
    _parseDeformFrame(rawData, frameStart, frameCount) {
        ...
        const rawVertices = DataParser.VERTICES in rawData ?
            ...
            (DataParser.VALUE in rawData ? rawData[DataParser.VALUE] : null);
        const offset = ObjectDataParser._getNumber(rawData, DataParser.OFFSET, 0); // uint
